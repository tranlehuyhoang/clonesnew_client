...
const Hoadon = () => {
    ...
    const [state, setstate] = useState(JSON.parse(localStorage.getItem('hoadon')));
    const addMoney = () => {
        try {
            const newMoneyAmount = Number(localStorage.getItem('money')) + Number(JSON.parse(localStorage.getItem('hoadon')).amount)
            axios.put('http://localhost:5000/api/user', { username: localStorage.getItem('user'), newMoneyAmount: newMoneyAmount });
    ...
    return (
        <div className='h-[100vh] w-[100%] flex    items-center   bg-white'>
            {state ?
                    ...
                    <div className='flex'>
                        <div>
                            <ul className="menu bg-base-100 w-56">
                                ...
                                <li><a>Giá tiền : {
                                    formattedAmount(Number(state.amount))
                                }</a></li>
                                <li><a>Tài khoản :104567890</a></li>
                                <li><a>Ngân hàng : MB Bank</a></li>
                                <li><a>Trạng Thái : Đang chờ thanh toán</a></li>
                                <li><a>Nội dung chuyển khoản :  {state.name}</a></li>
                            </ul>
                        </div>
                        <div>
                            <img src={state.qrcode} width={300} alt="" />

                        </div>
                    </div>
                    <Link to={'/'} onClick={addMoney}>quay lại</Link>
                </div>
                :
